...
const Home = () => {
  useEffect(() => {
    ...
    const wakeUpServer = async () => {
      try {
        console.log('≡ƒÜÇ Waking up server silently...');
        await apiCall(API_ENDPOINTS.HEALTH, {
          method: 'GET',
          signal: AbortSignal.timeout(10000) // 10 second timeout
        });
        console.log('Γ£à Server is awake');
      } catch (error) {
        console.error('Γ¥î Server health check failed:', error);
        ...
        setTimeout(async () => {
          try {
            ...
            console.log('Γ£à Server is awake (retry)');
          } catch (retryError) {
            console.error('Γ¥î Server health check retry failed:', retryError);
          }
        }, 5000);
      }
    };
